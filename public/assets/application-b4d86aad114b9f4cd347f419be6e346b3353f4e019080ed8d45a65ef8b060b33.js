...
(function() {
  $(document).ready(function() {
    return $("#cham").click(function() {
      $(".op-chamados").slideToggle();
    });
  });
...
(function() {
  $(document).on('ready page:load', function() {
    ...
    $('#chamado_categoria_id').change(function() {
      ...
      if (resultado) {
        ...
        return $('#chamado_objeto_id').html(resultado);
      } else {
        $('#chamado_objeto_id').empty;
        $("label[for='chamado_objeto_id']").hide();
        return $('#chamado_objeto_id').hide();
      }
    });
    return $('#botao_verificar').click(function() {
      var i, rf_solicitante, _i, _len, _ref, _results;
      $('#solicitante_nome').val('').prop("disabled", false);
      $('#solicitante_sector_id').val('').prop("disabled", false);
      $('#solicitante_email').val('').prop("disabled", false);
      $('#solicitante_ramal').val('').prop("disabled", false);
      rf_solicitante = $('#solicitante_rf').val();
      _ref = gon.solicitantes;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        i = _ref[_i];
        if (i["rf"] === rf_solicitante) {
          $('#solicitante_nome').val(i['nome']).prop("disabled", true);
          $('#solicitante_sector_id').val(i['sector_id']).prop("disabled", true);
          $('#solicitante_email').val(i['email']).prop("disabled", true);
          _results.push($('#solicitante_ramal').val(i['ramal']).prop("disabled", true));
        } else {
          _results.push(void 0);
        }
      }
...
(function() {
  ...
  $(document).on('ready page:load', function() {
    ...
    return $('#resolucao_contato_externo').change(function() {
      var selecionado;
      selecionado = $('#resolucao_contato_externo :selected').text();
      if (selecionado === "Sim") {
        $("#form_contato_externo").show();
        return $('#form_contato_externo').html(form);
      } else {
        $('#form_contato_externo').empty;
        return $('#form_contato_externo').hide();
      }
    });
  });
...
$(document).ready(
    function() {
        setInterval(function() {
            $('#avisos').load(location.href + " #avisos");
        }, 3000);
});
...
$(document).ready(
    function() {
        setInterval(function() {
            $('#index_chamados').load(location.href + " #index_chamados");
        }, 3000);
});
(function() {
